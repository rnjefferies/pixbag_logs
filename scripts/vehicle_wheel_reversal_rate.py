...
def plot_steering_angle(data, reversals, participant_id, condition, output_dir):
    ...
    plt.legend([steering_line, reversal_lines[0], reversal_dots[0]], 
               ['Vehicle Wheel Angle', 'Reversal Line', 'Reversal Start/End'])

    plt.xlabel('Time (seconds)')
    plt.ylabel('Steering Wheel Angle (radians)')
    title = f'Vehicle Wheel Angle with Reversals - Participant {participant_id}, Condition {condition}'
    plt.title(title)
    plt.grid(True)
...
def process_all_participants_data(data_logs_dir):
    ...
    results_df.to_csv(os.path.join(data_logs_dir, 'vehicle_wheel_reversals_summary.csv'), index=False)
